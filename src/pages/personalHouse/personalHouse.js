...
;(async function() {
  ...
  if(id) {
    const res = await houseDetailApi({ id });
    $('#personal-house-slot').html(tmpl({ data: res, id }))
  } else {
    $('#personal-house-slot').html(tmpl({
      options: {
  ...
  $('#addHouseBtn').on('click', function() {
    const params = parseString($('#houseForm').serialize());
